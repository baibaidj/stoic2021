...
@NECKS.register_module()
class DenseCLNeck3D(BaseModule):
    '''The non-linear neck in DenseCL.
        Single and dense in parallel: fc-relu-fc, conv-relu-conv
    '''
    def __init__(self, 
                 ...
                 num_grid=None, 
                 init_cfg=[
                     dict(type='Xavier', layer='Conv3d'), 
                     dict(type='Normal', layer='Linear'), 
                     ]
                     ):
        super(DenseCLNeck3D, self).__init__(init_cfg)
        ...
        self.with_pool = num_grid != None
        if self.with_pool:
            self.pool = nn.AdaptiveAvgPool3d((num_grid, num_grid, num_grid))
        self.mlp2 = nn.Sequential(
            nn.Conv3d(in_channels, hid_channels, 1), 
            nn.ReLU(inplace=True),
            nn.Conv3d(hid_channels, out_channels, 1))
    ...
    def forward(self, x):
        ...
        if self.with_pool:
            x = self.pool(x) # sxsxs
        x = self.mlp2(x) # sxs: bxdxsxs
        avgpooled_x2 = self.avgpool2(x) # 1x1: bxd
        x = x.view(x.size(0), x.size(1), -1) # bxdxs^3
        avgpooled_x2 = avgpooled_x2.view(avgpooled_x2.size(0), -1) # bxd
        return [avgpooled_x, x, avgpooled_x2]