...
class NiftiSaver:
    ...
    def save_batch(self, batch_data: Union[torch.Tensor, np.ndarray], meta_data: Optional[Dict] = None) -> None:
        ...
        for i, data in enumerate(batch_data):  # save a batch of files
            *prefix, suffix = self.output_postfix.split('_')
            self.output_postfix = '%s_%04d'%('_'.join(prefix), int(suffix) + i)
            self.save(data, {k: meta_data[k][i] for k in meta_data} if meta_data else None)
    

    def save_batch_gpu(self, batch_data: Union[torch.Tensor, np.ndarray], meta_data: Optional[Dict] = None) -> None:
        """
        Save a batch of data into Nifti format files.
        ...
        for i, data in enumerate(batch_data):  # save a batch of files
            *prefix, suffix = self.output_postfix.split('_')
            self.output_postfix = '%s_%04d'%('_'.join(prefix), int(suffix) + i)
            self.save(data, meta_data[i] if meta_data else None)
