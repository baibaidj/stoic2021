...
def compute_importance_map(
    ...
    """
    if verbose: print('\n IMAP mode', mode, 'device', device, 'size', patch_size)
    mode = BlendMode(mode)
    ...
    if mode == BlendMode.CONSTANT:
        ...
        sigmas = torch.tensor([i * sigma_s for i, sigma_s in zip(patch_size, sigma_scale)], device = device)
        if verbose: print_tensor('\tSigma', sigmas)
        ...
        if verbose: print_tensor('\tPostGaussian', importance_map)
    ...
    if verbose: print_tensor('\tfinal imap', importance_map)
